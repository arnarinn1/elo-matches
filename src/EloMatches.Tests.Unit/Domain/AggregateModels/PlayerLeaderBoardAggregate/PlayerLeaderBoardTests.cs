...
namespace EloMatches.Tests.Unit.Domain.AggregateModels.PlayerLeaderBoardAggregate
{
    ...
    public class PlayerLeaderBoardTests
    {
        ...
        [Test]
        public void UpdateLeaderBoardDescend_ShouldStayInSamePlace_WhenEloRatingOfAllPlayersAreEqualBelowHim()
        {
            var players = new List<PlayerOnLeaderBoard>
            {
                new PlayerOnLeaderBoard(_player1, 100, 1),
                new PlayerOnLeaderBoard(_player2, 85, 2),
                new PlayerOnLeaderBoard(_player3, 80, 3),
                new PlayerOnLeaderBoard(_player4, 80, 4),
                new PlayerOnLeaderBoard(_player5, 80, 5)
            };

            var leaderBoard = PlayerLeaderBoardReflectionFactory.Create(players);
            leaderBoard.UpdateLeaderBoard(new PlayerId(_player2), -5);
        ...
        [Test]
        public void UpdateLeaderBoardDescend_ShouldMoveIntoLastPlace_WhenEloDifferencePutsPlayerBelowLastPlayer()
        {
            var players = new List<PlayerOnLeaderBoard>
            {
                new PlayerOnLeaderBoard(_player1, 100, 1),
            ...
            leaderBoard.DomainEvents.Should().HaveCount(2);
            leaderBoard.VerifyPlayerDescendedOnLeaderBoard(_player4, 5, 4, null);
            leaderBoard.VerifyPlayerAscendedOnLeaderBoard(_player5, 4, 5, _player4);

            players.Single(x => x.PlayerId == _player1).Rank.Should().Be(1);
            players.Single(x => x.PlayerId == _player2).Rank.Should().Be(2);
            players.Single(x => x.PlayerId == _player3).Rank.Should().Be(3);
            players.Single(x => x.PlayerId == _player5).Rank.Should().Be(4);
            players.Single(x => x.PlayerId == _player4).Rank.Should().Be(5);
        }
